# raspbian-linux-9-ALL-lib.pl
# Networking functions for Raspbian linux using dhcpcd for network configuration
# This configuration is used in Raspbian 9
#
# Andreu Cladera, November 2008
# This code is based on debian-linux-lib.pl
...
foreach $iface (@ifaces) {
	my ($name, $addrfam, $method, $file_type, $options) = @$iface;
	if ($addrfam eq 'inet') {
		...
		foreach $option (@$options) {
			...
			if($file_type eq 'interfaces') {
				...
				if($param eq 'static ip_address') {
					if(my ($adr, $nmask) = ($value =~ /^\s*(\d+\.\d+\.\d+\.\d+)\/(\d+)\s*$/)) {
						$cfg->{'address'} = $adr;
						$cfg->{'netmask'} = prefix_to_mask($nmask);
					}
				} elsif($param eq 'static routers') {
					if(my ($gateway) = ($value =~ /^\s*(\d+\.\d+\.\d+\.\d+)/)) {
						$cfg->{'gateway'} = $gateway;
					}
				} elsif($param eq 'static domain_name_servers') {
					$cfg->{'dns-nameservers'} = $value;
				} elsif($param eq 'static domain_name') {
					$cfg->{'dns-domain'} = $value;
				} else {
					$cfg->{$param} = $value;
				}
			}
...
my $name = $cfg->{'virtual'} ne "" ? $cfg->{'name'}.":".$cfg->{'virtual'}
				       : $cfg->{'name'};
my $file_type = $cfg->{'file_type'} ? $cfg->{'file_type'} : "unknow";
...
foreach $iface (@ifaces) {
	local $address;
	if($iface->[3] eq 'interfaces') {
		foreach my $opt (@{$iface->[4]}) {
			if($opt->[0] eq 'address'){
				$address = $opt->[1];
				last;
				}
			}
	} elsif($iface->[3] eq 'dhcpcd') {
		foreach my $opt (@{$iface->[4]}) {
			if($opt->[0] =~ /static\s+ip_address$/) {
				$address = $opt->[1];
				last;
			}
		}
...
if ($file_type eq "unknow") {
	if(($in{'vlan'} == 1) || ($cfg->{'bond'} == 1) || ($cfg->{'virtual'} ne "")) {
		$file_type = 'interfaces';
	} else {
		$file_type = 'dhcpcd';
	}
}
...
if($file_type eq 'interfaces') {
	...
	@autos = get_auto_defs();
	$amode = $gconfig{'os_version'} > 3 || scalar(@autos);
	...
	if ($cfg->{'dhcp'} == 1) {
		...
		$method = 'static';
		if ($cfg->{'address'} && $cfg->{'netmask'}) {
			local $nmask = mask_to_prefix($cfg->{'netmask'});
			push(@options, ['static ip_address', $cfg->{'address'}.'/'.$nmask]);
		}
		if($cfg->{'gateway'}) {
			push(@options, ['static routers', $cfg->{'gateway'}]);
		}
		if($cfg->{'dns-nameservers'}) {
			push(@options, ['static domain_name_servers', $cfg->{'dns-nameservers'}]);
		}
		if($cfg->{'dns-domain'}) {
			push(@options, ['static domain_name', $cfg->{'dns-domain'}]);
		}
		if(!$cfg->{'address6'} && !$cfg->{'auto6'}) {
			push(@options, ['noipv6','']);
		}
	}
} else {
	error("Unknow file type: $file_type");
}
...
if (!$found) {
	...
	if($in{'vlan'} == 1) {
		&modify_interface_def($cfg->{'physical'}.'.'.$cfg->{'vlanid'},
				      'inet', $method, $file_type, \@options, 0);
		}
	else {
...
if (!$found6 && @options6) {
	...
	&new_interface_def($cfg->{'fullname'},
			   'inet6', $method, 'interfaces', \@options6);
	}
elsif ($found6 && @options6) {
	# Need to update IPv6 block
	&modify_interface_def($cfg->{'fullname'},
			      'inet6', $method, 'interfaces', \@options6, 0);
	}
elsif ($found6 && !@options6) {
	# Need to delete IPv6 block
	&delete_interface_def($cfg->{'fullname'}, 'inet6', 'interfaces');
	}

# Set auto option to include this interface, or not
if ($amode) {
	if ($cfg->{'up'} && $file_type eq 'interfaces') {
		if($in{'vlan'} == 1) {
...
my $cfg = $_[0];
	if($cfg->{'file_type'}) {
		&delete_interface_def($cfg->{'fullname'}, 'inet', $cfg->{'file_type'});
		if (@{$cfg->{'address6'}}) {
			&delete_interface_def($cfg->{'fullname'}, 'inet6', 'interfaces');
			}
		my @autos = get_auto_defs();
...
local ($d, @st, @hr);
foreach $d (@ifaces) {
	local ($name, $addrfam, $method, $file_type, $options) = @$d;
	local $o;
	local $onum = -1;
	foreach $o (@$options) {
		$onum++;
...
#    file_type     the file type where the interface is defined ('dhcpcd', 'interfaces')
...
# read primary dhcpcd configuration file
&open_readfile(CFGFILE, $dhcpcd_config) ||
	error("Unable to open $dhcpcd_config");
...
while (defined $line) {
	...
	if(my ($name) =($line =~ /\s*interface\s+(\S+)\s*$/)) {
		my @iface_options;
		my $addrfam = "inet";
		my $method = "dhcp";
		my $file_type = 'dhcpcd';
		$line = <CFGFILE>;
		while(defined $line && !($line =~ /^s*(interface|profile|allowinterfaces|denyinterfaces)/)) {
			chomp($line);
			...
			my ($param, $value);
			#static options
			if(($param, $value) = ($line =~ /^\s*static\s+(\S+)\s*=\s*(.+)\s*$/) ) {
				push(@iface_options, ["static $param", $value]);
				if($param == "ip_address") {
					$method = "static";
				}
				}
			elsif( ($param, $value) = ($line =~ /^\s*(\S+)\s+(.+)\s*$/) ) {
				...
				error("Error in option line in file $dhcpcd_config. Line: $line");
			}
			$line = <CFGFILE>;
		}
		push(@ret, [$name, $addrfam, $method, $file_type, \@iface_options]);
	} else {
		$line = <CFGFILE>;
	}
}
...
# read network/interfaces extra file for virtual interfaces
if (!&open_readfile(CFGFILE, $network_interfaces_config)) {
	webmin_debug_log("WARNING", "Unable to open $network_interfaces_config");
	return @ret;
}
# read the file line by line
$line = <CFGFILE>;
...
# the parameters should be (name, addrfam, method, file_type, options, mode)
...
if($file_type eq 'interfaces') {
	...
	local (*OLDCFGFILE, *NEWCFGFILE);
	&open_readfile(OLDCFGFILE, "$dhcpcd_config~") ||
		error("Unable to open $dhcpcd_config");
	&lock_file($dhcpcd_config);
	&open_tempfile(NEWCFGFILE, "> $dhcpcd_config", 1) ||
		error("Unable to open $dhcpcd_config");
	...
	my $new_options_wrote;
	while(defined($line=<OLDCFGFILE>)) {
		if(($inside_modify_region == 0) && ($line =~ /^\s*interface\s+$name\s*$/)) {
			# Start of the interface section to modify
			$inside_modify_region = 1;
			$iface_line = 1;
			$new_options_wrote = 0;
		} elsif(($inside_modify_region == 1) && (
		         ($line =~ /^\s*interface\s+\S+\s*$/) ||
		         ($line =~ /s*(profile|allowinterfaces|denyinterfaces)/))) {
		...
		if($line =~ /s*#/ || $line =~ /^\s*$/) {
			...
			# No inside modify region. Copy lines.
			&print_tempfile(NEWCFGFILE, $line);
		} else {
			# Should the interface line be changed or the options?
			if($iface_line ==1 && $mode == 0) {
				&print_tempfile(NEWCFGFILE, "interface $name\n");
			}
			# only write the new options and skip the old ones or just do
	        # nothing if mode is delete
	        # write only upon first entrance here
			if($mode == 0 && $new_options_wrote == 0) {
				...
				foreach $option (@$options) {
					...
					if($value ne '') {
						...
						&print_tempfile(NEWCFGFILE, "\t$param\n");
					}
				}
			}
		}
...
# the parameters should be (name, addrfam, method, file_type, options)
# with options being an array of (param, value) pairs
...
sub new_interface_def
{
	...
	if($file_type eq 'dhcpcd') {
		...
		copy("$dhcpcd_config", "$dhcpcd_config~");
		local *CFGFILE;
		&open_lock_tempfile(CFGFILE, ">> $dhcpcd_config") ||
			error("Unable to open $dhcpcd_config");
		&print_tempfile(CFGFILE, "\ninterface $name\n");
		if($method eq 'static') {
			foreach $option (@options) {
...
# the parameters should be (name, addrfam, file_type)
sub delete_interface_def
{
local ($name, $addrfam, $file_type) = @_;
&modify_interface_def($name, $addrfam, '', $file_type, [], 1);
...
sub apply_network
{
&system_logged("(cd / ; service networking restart) >/dev/null 2>&1");
&system_logged("(cd / ; service dhcpcd reload) >/dev/null 2>&1");
}
...
foreach $iface (grep { $_->[1] eq 'inet' } @ifaces) {
	foreach $o (@{$iface->[4]}) {
		...
		if ($o->[0] eq 'static routers') {
			local @routers = split(/\s+/,$o->[1]);
			return ( $routers[0], $iface->[0] );
			}
		}
	}
return ( );
...
sub set_default_gateway
{
	...
	foreach my $iface (grep { $_->[1] eq 'inet' } @ifaces) {
		...
		if($iface->[3] eq 'interfaces') {
			$iface->[4] = [ grep { $_->[0] ne 'gateway' } @{$iface->[4]} ];
		} elsif($iface->[3] eq 'dhcpcd') {
			$iface->[4] = [ grep { $_->[0] ne 'static routers'} @{$iface->[4]} ];
		}
		
		# Add if needed
		if ($iface->[0] eq $_[1]) {
			if($iface->[3] eq 'interfaces') {
				push(@{$iface->[4]}, [ 'gateway', $_[0] ]);
			} elsif($iface->[3] eq 'dhcpcd') {
				push(@{$iface->[4]}, [ 'static routers', $_[0] ]);
			}
		}
...
# os_get_dns_config(&dns)
#
# Returns the filename where dns info is obtained
...
sub os_get_dns_config
{
	...
	foreach my $i (@ifaces) {
		local $file_type = $i->[3];
		if($file_type eq 'interfaces') {
			local ($ns) = grep { $_->[0] eq 'dns-nameservers' } @{$i->[4]};
			local @dom = grep { $_->[0] eq 'dns-domain' ||
					    $_->[0] eq 'dns-search' } @{$i->[4]};
			if ($ns) {
...
if (@{$conf->{'domain'}} > 1) {
	...
	@domain_name = ( [ 'static domain_name', join(" ", @{$conf->{'domain'}}) ] );
	}
elsif (@{$conf->{'domain'}}) {
	@dns_domain  = ( [ 'dns-domain',  $conf->{'domain'}->[0] ] );
	@domain_name = ( [ 'static domain_name', $conf->{'domain'}->[0] ] );
	}
foreach my $i (@ifaces) {
	local $file_type = $i->[3];
	if($file_type eq 'interfaces') {
		...
		if ($ns) {
			if (@{$conf->{'nameserver'}}) {
				...
				$i->[4] = [ grep { $_->[0] ne 'nameservers' }
						 @{$i->[4]} ];
			}
			$i->[4] = [ grep { $_->[0] ne 'dns-domain' &&
					   $_->[0] ne 'dns-search' }
					 @{$i->[4]} ];
			push(@{$i->[4]}, @dnssearch);
		...
		if($ns) {
			if(@{$conf->{'nameserver'}}) {
				...
				$i->[4] = [ grep { !($_->[0] =~ /static\s+domain_name_servers$/) }
						@{$i->[4]} ];
			}
			$i->[4] = [ grep { !($_->[0] =~ /static\s+domain_name$/)}
					@{$i->[4]} ];
			push(@{$i->[4]}, @domain_name);
			&modify_interface_def($i->[0], $i->[1], $i->[2],
					      $i->[3], $i->[4], 0);
			$need_apply = 1;
		}
	...
	foreach my $i (@ifaces) {
		...
		if($file_type eq 'interfaces') {
			local ($a) = grep { $_->[0] eq 'address' &&
				    &check_ipaddress($_->[1]) } @{$i->[4]};
			next if (!$a);
			if (@{$conf->{'nameserver'}}) {
				push(@{$i->[4]}, [ 'dns-nameservers',
